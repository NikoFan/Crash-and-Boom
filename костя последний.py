...
SPEED = 8  # Скорость при перемещении
moving_speed = SPEED
...
picture = pygame.image.load('background\icon.png')  # Редактирование окна
...
def show_headband():
    ...
    while show:  # Запуск игры
        ...
        button_text(WIDTH / 1.527, HEIGHT // 2 + 110, 'Start Game', BLACK)
        button_text(WIDTH / 1.527, HEIGHT // 2, 'Store', BLACK)
...
def run_game():  # Основная функция
    ...
    global choiceX, user_down, user_up, userY, n_sit, stay_sit, right_sit, left_sit
    ...
    while game:  # Запуск игры
        ...
        life_bar_x = enemyX + (enemyWidth - enemy_health) / 2
        life_bar_y = enemyY - 10

        display.blit(land, (0, 0))
        set_target()
        display.blit(enemyUp[enemyID][m], (enemyX, enemyY))
        pygame.draw.rect(display, RED, (life_bar_x, life_bar_y, enemy_health, 10))
        pygame.draw.rect(display, GREEN, (life_bar_x, life_bar_y, (enemy_health - killcount), 10))

        if (keys[pygame.K_LSHIFT] and not (keys[pygame.K_a]) and not (keys[pygame.K_d])) or (
                keys[pygame.K_LSHIFT] and keys[pygame.K_a] and keys[pygame.K_d]):
            stay_sit = True
            user_sit_down()
            moving_speed = SPEED - 7
...
def user_sit_down():
    ...
    if stay_sit:
        ...
        sit_anim_counter1 += 1
    if left_sit:
        if sit_anim_counter2 == -7:
            sit_anim_counter2 = -1
            left_sit -= 1
        sit_anim_counter2 -= 1
        time.sleep(0.01)
    if right_sit:
        if sit_anim_counter2 == 6:
            sit_anim_counter2 = 0
            right_sit += 1
        sit_anim_counter2 += 1
        time.sleep(0.01)
...
def button_text(x, y, message, font_color, font_type='TEXT.otf', font_size=72):
    global button_color, menu_background, width_button, height_button
...
def set_target():
    global enemyY, enemyX, userX, userY, userWidth, userHeight
    if True:  # Запуск игры
        if userX + userWidth / 2 > enemyX:
            enemyX += 1
        if userX + userWidth / 2 < enemyX:
            enemyX -= 1
        if userY > enemyY:
